...
#         print(f"batch is: {iter}")
        batch.split = 'train'
        batch.to(torch.device(cfg.accelerator))
#         pred, true = model(batch)
        pred_true, inter_A = model(batch)
        pred, true = pred_true
        ...
        #### regularizer loss w/out cutting off backprop####
        reg = float(cfg.reg)
#         print(reg)
#         print(loss) ## original
        ...
        if reg < 0:
            for int_A in inter_A:
#                 print(-reg * F.mse_loss(int_A, dense_adj))
#                 loss = loss - reg * F.mse_loss(int_A, dense_adj) ## reverse reg
#                 loss = loss - reg * F.cross_entropy(int_A, dense_adj, reduction="mean") ## reverse reg
                if cfg.loss == "CE":
                    loss = loss - reg * F.cross_entropy(int_A, dense_adj) ## reverse reg
                elif cfg.loss == "L1":
                    loss = loss - reg * F.l1_loss(int_A, dense_adj) ## reverse reg
                else:
                    raise IndexError
#             print(f"+regularized {loss}")
#             print("----")
            loss.backward()
        elif reg > 0:
            #### one step regularizer loss ####
            loss.backward(retain_graph=True) 
            gradients = {}
            ## store to freeze all gradients
    ...
    #                 print(name, p)
            ...
            optimizer.zero_grad() 
            loss = 0
            for int_A in inter_A: ## compute regularizer
#                 print(reg * F.l1_loss(int_A, dense_adj))
#                 loss = loss + F.l1_loss(int_A, dense_adj)
                if cfg.loss == "CE":
                    loss = loss + reg * F.cross_entropy(int_A, dense_adj) ## reverse reg
                elif cfg.loss == "L1":
                    loss = loss + reg * F.l1_loss(int_A, dense_adj) ## reverse reg
                    ...
                    raise IndexError
#                 loss = loss + reg * F.cross_entropy(int_A, dense_adj) ## reverse reg
#                 loss = loss + reg * compute_loss(int_A, dense_adj)[0]
#             print(f"+regularized {loss}")
#             print("----")
            loss.backward()
            for name, p in model.named_parameters(): ## store to freeze all gradients
                if p.grad is not None or name in gradients:
                    if ".Q." not in name and ".K." not in name or p.grad is None: # and ".K." not in name:
                        p.grad = gradients[name] ## ignore second forward if not Q, K, V(inadvertently)
                    else:
#                         print(name, p.grad)
...
#             pred, true = model(batch)
            (pred, true, extra_stats), _ = model(batch)
#             extra_stats = {}
        else:
            (pred, true), _ = model(batch)
#             pred_true, _ = model(batch)
#             pred, true = pred_true
            extra_stats = {}
        if cfg.dataset.name == 'ogbg-code2':
            loss, pred_score = subtoken_cross_entropy(pred, true)
